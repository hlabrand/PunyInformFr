...
#EndIf;
	...
	while(p_parse_pointer --> 0 == 'un' or 'une' or 'le' or 'la' or 'les' or 'des' or 'de') {
		wn = wn + 1;
		p_parse_pointer = p_parse_pointer + 4;
	}
...
[ _GetNextNoun p_parse_pointer p_phase _noun _oldwn _num_words_in_nounphrase _pluralword _i _j _k _all_found;
	...
	! skip 'the', 'all' etc
	while(p_parse_pointer --> 0 == 'un' or 'une' or 'le' or 'la' or 'de' or 'des' or 'du' or 'les' or ALL_WORD or EXCEPT_WORD1 or EXCEPT_WORD2) {
		if(p_parse_pointer --> 0 == ALL_WORD) {
			parser_all_found = true; !TODO: do we really need the global?
			_all_found = true;
		} else {
                    ! si on finit par "la", on veut dire le pronom, donc il ne faut pas sauter
                    if (wn == parse->1) { break;}
                }
...
#Endif;
	...
	! check for pronouns
	! Punyfr: trop difficile de prendre en charge "y" ou "lui"
	if(p_parse_pointer --> 0 == 'le' or 'la' or 'les') {
		...
		if(_noun == 0) {
			...
			if(p_phase == PHASE2) {
				print "Je ne sais pas à quoi ~",(address) p_parse_pointer --> 0, "~ fait référence.^";
				return -2;
			}
		} else if(TestScope(_noun) == false) {
			phase2_necessary = PHASE2_ERROR;
			if(p_phase == PHASE2) {
				print "Vous ne pouvez pas ~",(address) p_parse_pointer --> 0, "~ (", (name) _noun, ") voir présentement.^";
				return -2;
			}
	 	}
...
[ _IsSentenceDivider p_parse_pointer;
	...
	if(p_parse_pointer --> 0 == './/' or ',//' or 'et' or 'puis') rtrue;
	rfalse;
];
...
!	print "le ", himobj, " la ", herobj, " les ", themobj, "^";
];
	...
	if(consult_from && action==##Answer or ##Ask) {
		if(0 == noun or second) {
			for(_i=0 : _i < consult_words : _i++) {
				...
				if(_noun ~= 'un' or 'une' or 'le' or 'la' or 'les' or 'des' or 'de') {
					if(noun==0)
						noun = _noun;
					else
						second = _noun;
...
! PunyFr : addition
! Utilisons cette routine pour résoudre les commandes commençant par des pronoms: me lever, le prendre, etc.
[ BeforeParsing      _temp _i;
    ! ne pas le faire si on est dans une phase de désambiguation, i.e. "prendre la clé rouge ou la clé jaune" : "la jaune"
    if (phase2_necessary ~= PHASE2_DISAMBIGUATION) {
        if (parse->1 > 1 && parse-->1 == 'le' or 'la' or 'les') {
            ! Il faut inverser les deux premiers, dans buffer (au cas où une partie de l'input doit être écrite)...
            ! (on peut utiliser le tableau buffer2, il est toujours écrasé par les gens qui en ont besoin)
            _CopyInputArray(buffer, buffer2);
            _temp = WordLength(1);
            for (_i=0: _i<_temp: _i++) {
                buffer->_i = WordAddress(1)->_i;
            }
            buffer->_temp = ' ';
            for (_i=0: _i<WordLength(0): _i++) {
                buffer->(_i+_temp+1) = buffer2->_i;
            }
            ! ... puis dans parse
            _CopyParseArray(parse, parse3);
            for (_i=0: _i<4: _i++) {
                parse->(2+_i) = parse3->(6+_i);
                parse->(6+_i) = parse3->(2+_i);
            }
        }
    }
];
