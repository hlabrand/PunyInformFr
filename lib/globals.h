...
Constant ALL_WORD     = 'tout';
Constant EXCEPT_WORD1 = 'sauf';
Constant EXCEPT_WORD2 = 'sauf';
Constant AND_WORD     = 'et';
Constant THEN1__WD    = 'puis';
Constant comma_word   = 'virgule,';  ! An "untypeable word" used to substitute
...
Constant SOMEDIRECTION_STR = "(une direction)";
...
#IfV5;
Array _direction_dict_words static --> 'n//' 's//' 'e//' 'o//' 'ne' 'no' 'se' 'so' 'h//' 'b//' 0 0
	'nord' 'sud' 'est' 'ouest' 'nordest' 'nordouest' 'sudest' 'sudouest' 'haut' 'bas' 'dedans' 'dehors';
...
Array direction_name_array static --> "direction" "nord" "sud" "est" "ouest" "nord-est" "nord-ouest" "sud-est" "sud-ouest" "haut" "bas" "dedans" "dehors";
...
Object Directions
	with
		description "Regarder dans cette direction ne vous apprend rien.",
		short_name [;
...
#IfNot;
			...
			if(normal_directions_enabled) {
#Ifndef OPTIONAL_SHIP_DIRECTIONS;
				@je _w 'dehors' ?match_out;
				@je _w 'dedans' ?match_in;
				@je _w 'b//' 'bas' ?match_d;
!				@je _w 'terre' 'par' ?match_d;
				@je _w 'h//' 'haut' ?match_u;
#Endif;
#Ifdef OPTIONAL_FULL_DIRECTIONS;
				@je _w 's-e' 'sud-est' 'sudest' ?match_se;
				@je _w 's-o' 'sud-ouest' 'sudouest' ?match_sw;
				@je _w 'n-e' 'nordest' ?match_ne;
				@je _w 'n-o' 'nordouest' ?match_nw;
                                ! besoin de distinguer "nord-est" de "nord-ouest"
                                if (_w == 'nord-') {
                                    _c = WordAddress(wn)->5;
                                    if (_c == 'o') { jump match_nw;}
                                    if (_c == 'e') { jump match_ne;}
                                }
#Endif;
				@je _w 'e//' 'l^est' 'est' ?match_e;
				@je _w 'o//' 'l^ouest' 'ouest' ?match_w;
				@je _w 's//' 'sud' ?match_s;
				@je _w 'n//' 'nord' ?match_n;
			}
