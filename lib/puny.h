...
[ _PrintObjName p_obj p_form;
	if(p_obj hasnt proper) {
		if(p_form == FORM_CDEF or FORM_DEF) {
                        if (p_form == FORM_CDEF) { print "L"; } else { print "l"; }
                        if (p_obj has pluralname) { print "es "; }
                        else { if (p_obj has female) { print "a "; } else { print "e "; } }
		} else if(p_form == FORM_INDEF) {
			if(p_obj.&article) {
				PrintOrRun(p_obj, article, true);
				!print " ";                 ! on n'affiche pas un espace automatiquement à cause de l'élision
			}
			else {
                            if(p_obj has pluralname) { print "des "; }
                            else {
                                print "un"; if (p_obj has female) { print "e";} print " ";
                            }
                        }
		}
...
[ _PrintAfterEntry p_obj;
	if(p_obj has container && P_obj hasnt open) print " (fermé", (es) p_obj, ")";
	if(p_obj has container && (p_obj has open || p_obj has transparent)) {
		if(child(p_obj) == nothing) {
			print " (vide"; if (p_obj has pluralname) { print "s"; } print ")";
                } else {
			if(PrintContents(" (contenant ", p_obj)) print ")";
                }
	}
	if(p_obj has supporter && child(p_obj) ~= nothing) {
		if(PrintContents(" (soutenant ", p_obj)) print ")";       ! moche mais neutre en genre, à bon entendeur...
	}
!	if(p_obj has light && action == ##Inv) print " (qui fait de la lumière)";
#Ifndef OPTIONAL_NO_DARKNESS;
	if(p_obj has light) print " (qui fait de la lumière)";
#Endif;
	if(p_obj has worn && action == ##Inv) print " (porté", (es) p_obj, ")";
];
...
Object selfobj "vous"
	with
		name 'moi' 'moi-meme' 'meme' 'me',
		short_name  "vous-même",
		description "Votre apparence est aussi agréable qu'à l'accoutumée.",
		before NULL,
		after NULL,
		life NULL,
		each_turn NULL,
...
#Ifndef OPTIONAL_NO_DARKNESS;
Object thedark "Obscurité"
	with
		initial 0,
		description "Il fait noir ici !",
 		short_name 0;
#Endif;
...
#Endif;
		...
		if(AMUSING_PROVIDED == 0 && deadflag == 2 && verb_word == 'amusant') Amusing();
		if(verb_word == 'quitter') @quit;
#IfDef OPTIONAL_FULL_SCORE;
		if(verb_word == 'liste') FullScoreSub();
#EndIf;
	}
];
